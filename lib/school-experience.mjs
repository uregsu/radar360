...
const DATABASE_DERIVED_FIELDS = new Set([
  "average_score", "attention_score", "base_attention_level", "attention_level",
  "critical_dimension", "critical_dimension_score", "best_dimension", "best_dimension_score",
  "trigger_count", "below_3_count", "below_4_count", "below_5_count", "below_6_count",
]);

export function toSchoolExperienceInsert(row, importedBy) {
  return {
    ...Object.fromEntries(Object.entries(row).filter(([key]) => !DATABASE_DERIVED_FIELDS.has(key))),
...
const attentionLevelRank = (value) => ATTENTION_LEVELS.find((level) => level.value === value)?.rank ?? -1;
const numericDifference = (left, right, direction = "desc") => {
  const difference = Number(left ?? 0) - Number(right ?? 0);
  return direction === "desc" ? -difference : difference;
};

export function compareExperiencePriorities(left, right) {
  return numericDifference(attentionLevelRank(left.attention_level), attentionLevelRank(right.attention_level), "desc")
    || numericDifference(left.below_3_count, right.below_3_count, "desc")
    || numericDifference(left.below_4_count, right.below_4_count, "desc")
    || numericDifference(left.below_5_count, right.below_5_count, "desc")
    || numericDifference(left.below_6_count, right.below_6_count, "desc")
    || numericDifference(left.trigger_count, right.trigger_count, "desc")
    || numericDifference(left.attention_score, right.attention_score, "desc")
    || numericDifference(left.critical_dimension_score, right.critical_dimension_score, "asc")
    || numericDifference(left.average_score, right.average_score, "asc")
    || String(left.schools?.name ?? "").localeCompare(String(right.schools?.name ?? ""), "pt-BR")
    || String(left.id ?? "").localeCompare(String(right.id ?? ""), "pt-BR");
}

export function projectExperiencePriorities(metrics, referencePeriod) {
  return metrics.filter((metric) => metric.reference_period === referencePeriod).sort(compareExperiencePriorities);
}

export function toggleExperienceLevel(currentLevel, selectedLevel) {
  return currentLevel === selectedLevel ? "" : selectedLevel;
}

export function countExperienceLevels(metrics, referencePeriod) {
  const rows = metrics.filter((metric) => metric.reference_period === referencePeriod);
  return Object.fromEntries(ATTENTION_LEVELS.map(({ value }) => [
    value,
    rows.filter((metric) => metric.attention_level === value).length,
  ]));
}

export function filterExperienceMetrics(metrics, filters) {
  const {
    referencePeriod, query = "", level = "", dimension = "", trigger = "",
    averageMin = "", averageMax = "",
  } = filters;
  const normalizedQuery = query.toLocaleLowerCase("pt-BR");
  return metrics.filter((metric) => metric.reference_period === referencePeriod
    && (!query || (metric.schools?.name ?? "").toLocaleLowerCase("pt-BR").includes(normalizedQuery))
    && (!level || metric.attention_level === level)
    && (!dimension || metric.critical_dimension === dimension)
    && (!trigger || (trigger === "yes" ? metric.trigger_count > 0 : metric.trigger_count === 0))
    && (!averageMin || Number(metric.average_score) >= Number(averageMin))
    && (!averageMax || Number(metric.average_score) <= Number(averageMax)));
}

export function contextualExperienceClassification(value) {
  const score = Number(value);
  if (score < 3) return "Crítica";
  if (score < 5) return "Atenção elevada";
  if (score < 6) return "Acompanhamento";
  if (score >= 8) return "Favorável";
  return "Regular";
}

export function experienceHeatmapTooltip(school, dimension, value) {
  return {
    school: String(school),
    dimension: String(dimension),
    value: Number(value),
    classification: contextualExperienceClassification(value),
  };
}
