...
import { getActiveSector, getVisibleSectors, isSectorPath, SECTOR_INDEX_ROUTE } from "../lib/sector-navigation";
import { SectorNavigation } from "./SectorNavigation";
import { useSectorScope } from "./useSectorScope";
import { useSidebarDrawer } from "./useSidebarDrawer";
import { PAINEL_MDI_ROUTE } from "../config/products";
import { ASURE_OUVIDORIAS_EMBED_URL, ASURE_OUVIDORIAS_ROUTE } from "../config/ouvidorias";
import { demoData } from "../lib/demo/data";
import { demoProvider } from "../lib/demo/provider";
import { canAccessNavigation, canAccessAsureOuvidorias } from "../lib/permissions";
...
import SchoolExperience360, { SchoolExperienceSummary } from "./SchoolExperience360";
import { SchoolQualityOverview, SchoolQualitySummary, useSchoolQualityData } from "./SchoolQuality";
...
const nav = [
  ...
  ["◎","Experiência Escolar","/radar360/experiencia-escolar",["ADMIN","GESTAO","ESCOLA"]],
...
function navigationGroup(route: string) {
  ...
  if ([PAINEL_MDI_ROUTE, "/radar360/dirigente", "/radar360/gestao", "/radar360/escolas", "/radar360/matriz", "/radar360/experiencia-escolar"].includes(route)) return "Painéis estratégicos";
...
function Sectors({ go,user,availableSectors }: { go:(p:string)=>void;user:User;availableSectors:Sector[] }) {
  const [query,setQuery]=useState("");
  const [hubs,setHubs]=useState<HubRow[]>([]);
  useEffect(()=>{if(user.role==="VISITANTE"||!availableSectors.length)return;let current=true;void getSupabaseBrowserClient().from("hubs").select("id,name,external_url,description,status,integration_type,sector_id,sectors(code,name,slug)").eq("active",true).in("sectors.code",availableSectors.map(s=>s.label)).order("name").then(({data})=>{if(current)setHubs((data??[]) as unknown as HubRow[])});return()=>{current=false}},[user.id,user.role,availableSectors]);
  const filtered=availableSectors.filter(s=>(s.label+" "+s.name).toLowerCase().includes(query.toLowerCase()));
  const visibleHubs=(user.role==="VISITANTE"?demoHubCatalog:hubs).filter(h=>availableSectors.some(s=>s.label===("sectors" in h?h.sectors?.code:h.sector_id)));
  return <div className="operational-page"><PageTitle eyebrow="ARQUITETURA INSTITUCIONAL" title="Setores e hubs" text="Responsabilidades, produtos digitais e acessos organizados por contexto institucional."/>
    ...
    <section className="operational-section"><SectionHeader title="Setores institucionais" description="Selecione um setor para consultar áreas funcionais, responsabilidades e produtos vinculados."/>{filtered.length?<div className="sector-grid operational-sector-grid">{filtered.map(s=>{const integrated=user.role==="VISITANTE"?demoHubCatalog.some(h=>h.sector_id===s.shortName):hubs.some(h=>h.sectors?.code===s.shortName);return <button className="sector-card operational-sector-card" key={s.id} onClick={()=>go(s.route)}><div className="sector-head"><i aria-hidden="true">{s.icon}</i><SuperBIStatusBadge tone={integrated?"success":"neutral"}>{integrated?"Hub integrado":s.status}</SuperBIStatusBadge></div><span className="superbi-eyebrow">Setor institucional</span><h3>{s.label}</h3><b>{s.name}</b><p>{s.description}</p><footer><span>{integrated?"Produto digital vinculado":`${s.menu.length} áreas funcionais`}</span><b aria-hidden="true">→</b></footer></button>})}</div>:<SuperBIEmptyState title="Nenhum setor encontrado" description={availableSectors.length?"Ajuste o termo pesquisado para consultar a arquitetura institucional.":"Nenhum setor está disponível para o seu perfil."}/>}</section>
    <section className="operational-section"><SectionHeader title="Hubs e produtos vinculados" description="Ambientes operacionais preservam autoria, integração e forma de acesso."/><div className="hub-card-grid">{visibleHubs.map(h=><HubAccessCard key={h.id} hub={h} demo={user.role==="VISITANTE"}/>)}</div></section></div>;
}

function OuvidoriasModule() {
  const [loaded,setLoaded]=useState(false);
  const [failed,setFailed]=useState(false);
  const configured=Boolean(ASURE_OUVIDORIAS_EMBED_URL);
  return <div className="ouvidorias-page"><PageTitle eyebrow="ASURE · PROCESSOS E EXPEDIENTES" title="Ouvidorias" text="Monitoramento administrativo de manifestações, encaminhamentos, prazos e respostas." badge="Acesso restrito · Dirigente e setores administrativos"/>
    <section className="ouvidorias-source" aria-label="Informações da fonte"><span><b>Fonte</b>Planilha de Controle de Ouvidorias</span><span><b>Responsável</b>ASURE</span><span><b>Origem</b>Google Sheets</span><span><b>Atualização</b>Automática via fonte conectada</span><span><b>Acesso</b>Restrito</span></section>
    {!configured?<SuperBIEmptyState title="Painel de Ouvidorias em configuração" description="A integração com o painel de monitoramento da ASURE será disponibilizada neste espaço."/>:<section className="ouvidorias-panel" aria-labelledby="ouvidorias-panel-title">
      <header><div><span className="superbi-eyebrow">PAINEL GERENCIAL</span><h2 id="ouvidorias-panel-title">Controle de Ouvidorias</h2><p>Os dados são atualizados conforme a política de atualização da fonte conectada ao Looker Studio.</p></div><a href={ASURE_OUVIDORIAS_EMBED_URL} target="_blank" rel="noopener noreferrer">Abrir painel completo <span aria-hidden="true">↗</span></a></header>
      <div className="ouvidorias-frame-wrap">
        {!loaded&&!failed&&<LoadingState title="Carregando painel de Ouvidorias…" description="Conectando ao ambiente institucional do Looker Studio."/>}
        {failed?<SuperBIEmptyState title="Não foi possível carregar o painel" description="Abra o painel completo ou tente novamente mais tarde."/>:<iframe className={loaded?"is-loaded":""} src={ASURE_OUVIDORIAS_EMBED_URL} title="Painel de Ouvidorias da ASURE" loading="lazy" allowFullScreen sandbox="allow-storage-access-by-user-activation allow-scripts allow-same-origin allow-popups allow-popups-to-escape-sandbox" onLoad={()=>setLoaded(true)} onError={()=>setFailed(true)}/>}
      </div>
    </section>}
  </div>;
}

function SectorView({ sector,user,go }: { sector: Sector;user:User;go:(p:string)=>void }) {
  ...
  return <><PageTitle eyebrow="PAINEL 360 DO SETOR" title={`${sector.shortName} · ${sector.name}`} text={sector.description}/>
    <div className="sector-layout"><aside className="subnav"><span>ÁREAS FUNCIONAIS</span>{sector.menu.map(m=><Fragment key={m}><button className={m===active?"active":""} onClick={()=>setActive(m)}>{m}<b>›</b></button>{m==="Processos e Expedientes"&&sector.shortName==="ASURE"&&canAccessAsureOuvidorias(user.role)&&<button className="subnav-child" onClick={()=>go(ASURE_OUVIDORIAS_ROUTE)}>Ouvidorias<b>›</b></button>}</Fragment>)}</aside>
...
function SchoolView({ school, user }: { school:School; user:User }) {
  const demo=user.role==="VISITANTE";
  const tabs=["Resumo","Supervisão","Pedagógico","Qualidade da Aula","Tecnologia","Rede Escolar","Vida Escolar","Matrícula","Pessoas","Administração e Finanças","Obras e Manutenção","Evidências","Demandas"];
  ...
  const schoolQuality=useSchoolQualityData(realSchool.id,demo);
  ...
  return <><PageTitle eyebrow="PAINEL 360 DA ESCOLA" title={realSchool.name} text="Visão integrada da unidade escolar, respeitando as permissões de cada domínio." badge={realSchool.pei?"PEI":undefined}/>
    ...
    {tab==="Resumo"&&<div className="school-360-layers"><SchoolQualitySummary state={schoolQuality} onOpenDetails={()=>setTab("Qualidade da Aula")}/>{!demo&&realSchool.id&&<SchoolExperienceSummary schoolId={realSchool.id}/>}</div>}<DataProvenance demo={demo}/></>;
}
...
export function AppShell({ user, onLogout }: { user:User; onLogout:()=>void }) {
  ...
  const sectorScope=useSectorScope(user);
  const availableSectors=useMemo(()=>getVisibleSectors(user,sectorScope.key),[user,sectorScope.key]);
  const closeMobile=useCallback(()=>setMobile(false),[]);
  const {compact,sidebarRef,triggerRef,closeRef}=useSidebarDrawer(mobile,closeMobile);
  // Every destination is checked before mounting content, including direct URLs and popstate.
  const go=(next:string)=>{window.history.pushState({},"",next);setPath(next);setMobile(false);window.scrollTo(0,0)};
  useEffect(()=>{const fn=()=>setPath(pathNow());window.addEventListener("popstate",fn);return()=>window.removeEventListener("popstate",fn)},[]);
  const sector=useMemo(()=>getActiveSector(path),[path]);
  ...
  const content=sectorScope.loading&&isSectorPath(path)&&path!==ASURE_OUVIDORIAS_ROUTE?<LoadingState title="Verificando acesso ao setor…" description="Consultando seu vínculo institucional."/>:!canAccessNavigation(user,path,sectorScope.key)?<ModulePage path="/denied" go={go}/>:path==="/dashboard"||path==="/"?<Overview user={user} go={go}/>:path==="/radar360/experiencia-escolar"?<SchoolExperience360 user={user} go={go}/>:path===ASURE_OUVIDORIAS_ROUTE?<OuvidoriasModule/>:path===PAINEL_MDI_ROUTE?<PainelMdiModule/>:path==="/radar360"?<Radar360Module user={user} go={go} availableSectors={availableSectors}/>:path==="/radar360/meu-setor"?<ManagementSectorDashboard user={user}/>:path==="/radar360/minha-escola"?<SchoolView school={schoolSelf} user={user}/>:path==="/radar360/usuarios"?<UsersAdmin/>:path==="/radar360/demandas"?<Demands user={user} recordType="DEMANDA" go={go}/>:itemId?<ItemDetailModule id={itemId} user={user} go={go}/>:path==="/radar360/itens"||itemType?<Demands user={user} recordType={itemType} go={go}/>:path.replace(/\/+$/,"")===SECTOR_INDEX_ROUTE?<Sectors key={user.id} go={go} user={user} availableSectors={availableSectors}/>:sector?<SectorView key={sector.key} sector={sector} user={user} go={go}/>:path==="/radar360/escolas"?<Schools go={go} user={user}/>:school?<SchoolView school={school} user={user}/>:path==="/radar360/biguarulho"?<BigModule/>:path==="/radar360/integracao"?<MdiModule/>:path==="/radar360/comunica"?<CommunicaModule user={user}/>:path==="/radar360/evidencias"?<EvidenceModule user={user}/>:path==="/radar360/dirigente"?<ExecutiveModule user={user} mode="ADMIN"/>:path==="/radar360/gestao"?<ExecutiveModule user={user} mode="GESTAO"/>:path==="/radar360/matriz"?<IntegrationMatrixModule/>:path==="/radar360/acordos"?<AgreementsModule/>:path==="/radar360/sobre"?<AboutModule/>:path==="/acesso-negado"?<ModulePage path="/denied" go={go}/>:<ModulePage path={path} go={go}/>;
  const managementNav=[["⌂","Visão Regional 360","/dashboard"],["◉","SuperBI 360 | GSU","/radar360"],["◆","Meu Setor","/radar360/meu-setor"],["▦","Setores e Hubs","/radar360/setores"],["◆","BIGuarulhosSul","/radar360/biguarulho"],["↗","Integração! MDI","/radar360/integracao"],["◫","Comunica!","/radar360/comunica"],["✓","Evidências","/radar360/evidencias"],["◎","Experiência Escolar","/radar360/experiencia-escolar"],["◇","Painel MDI",PAINEL_MDI_ROUTE],["◈","Painel da Gestão","/radar360/gestao"],["⌂","Painel das Escolas","/radar360/escolas"],["≋","Demandas","/radar360/demandas"],["⊞","Matriz de Integração","/radar360/matriz"],["≋","Acordos","/radar360/acordos"],["ⓘ","Sobre","/radar360/sobre"]] as const;
  const schoolNav=[["⌂","Minha Escola","/radar360/minha-escola"],["◈","Visão geral","/dashboard"],["◎","Experiência Escolar","/radar360/experiencia-escolar"],["≋","Demandas","/radar360/demandas"],["◆","Acompanhamentos","/radar360/itens/acompanhamento"],["✓","Evidências","/radar360/evidencias"],["◫","Devolutivas","/radar360/evidencias"],["◉","Orientações","/radar360/comunica"],["▦","Documentos","/radar360/comunica"],["ⓘ","Sobre","/radar360/sobre"]] as const;
  ...
  const currentLabel=itemId?"Detalhe institucional":sector?sector.label:school?school.name:currentNav?.[1]||"SuperBI 360 | GSU";
  return <div className="app-shell">
    <aside id="main-sidebar" ref={sidebarRef} className={`sidebar ${mobile?"open":""}`} aria-label="Navegação principal" inert={compact&&!mobile} aria-hidden={compact&&!mobile?true:undefined}><SuperBIBrand size="sm" tone="light"/><button ref={closeRef} className="close-menu" onClick={closeMobile} aria-label="Fechar navegação">×</button><nav>{orderedNav.map((n,index)=>{const group=navigationGroup(n[2]);const previous=index?navigationGroup(orderedNav[index-1][2]):"";return <Fragment key={`${n[2]}-${n[1]}`}>{group!==previous&&previous==="Visão institucional"&&<SectorNavigation key={sector?.key??"overview"} sectors={availableSectors} activeKey={sector?.key} onNavigate={go}/>}{group!==previous&&<span className="nav-heading">{group}</span>}<button className={path===n[2]||n[2]!=="/radar360"&&n[2]!==SECTOR_INDEX_ROUTE&&path.startsWith(n[2]+"/")?"active":""} onClick={()=>go(n[2])} aria-current={path===n[2]?"page":undefined}><i aria-hidden="true">{n[0]}</i><span>{n[1]}</span></button></Fragment>})}</nav><div className="sidebar-foot"><span>URE GUARULHOS SUL</span><b>SuperBI 360 | GSU</b><small>Inteligência regional</small></div></aside>
    <div className="app-main"><header className="topbar"><button ref={triggerRef} className="hamburger" onClick={()=>setMobile(true)} aria-label="Abrir navegação" aria-expanded={mobile} aria-controls="main-sidebar">☰</button><div className="topbar-context"><span>SuperBI 360 | GSU</span><strong>{currentLabel}</strong></div><div className="topbar-date"><span>Hoje</span><strong>{new Intl.DateTimeFormat("pt-BR",{weekday:"long",day:"2-digit",month:"long"}).format(new Date())}</strong></div>
