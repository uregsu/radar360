...
function Login({ onDemo, onAuthenticated }: { onDemo: () => void; onAuthenticated: () => Promise<void> }) {
  const [mode,setMode]=useState<"login"|"forgot"|"reset">(()=>pathNow()==="/redefinir-senha"?"reset":"login");
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if(mode==="forgot"){
      const {error:authError}=await supabase.auth.resetPasswordForEmail(email,{redirectTo:`${window.location.origin}/redefinir-senha`});
      if(authError)setError(authError.message);else setMessage("Enviamos as instruções de recuperação para o e-mail informado.");
    }else if(mode==="reset"){
      const {error:authError}=await supabase.auth.updateUser({password});
      if(authError)setError(authError.message);else{setMessage("Senha atualizada com segurança.");window.history.replaceState({},"","/dashboard");await onAuthenticated()}
    }else{
      const {error:authError}=await supabase.auth.signInWithPassword({email,password});
      if(authError)setError("E-mail ou senha inválidos.");else await onAuthenticated();
    }
    setLoading(false);
  };
  ...
  return <main className="login-shell">
    ...
    <section className="login-side">
      <form className="login-card" onSubmit={submit}>
        ...
        <h2>{mode==="forgot"?"Recuperar acesso":mode==="reset"?"Definir nova senha":"Bem-vindo ao RADAR 360"}</h2>
        <p>{mode==="forgot"?"Informe seu e-mail institucional.":mode==="reset"?"Cadastre uma nova senha segura.":"Acesse o ambiente integrado da URE Guarulhos Sul."}</p>
        {mode!=="reset"&&<label>E-mail institucional<div className="input-wrap"><span>＠</span><input aria-label="E-mail institucional" value={email} onChange={e=>setEmail(e.target.value)} type="email" required/></div></label>}
        {mode!=="forgot"&&<label>{mode==="reset"?"Nova senha":"Senha"}<div className="input-wrap"><span>⌁</span><input aria-label={mode==="reset"?"Nova senha":"Senha"} value={password} onChange={e=>setPassword(e.target.value)} type="password" minLength={8} required/></div></label>}
        {error && <div className="login-error">{error}</div>}
        {message&&<div className="login-success">{message}</div>}
        <button className="primary-btn" disabled={loading}>{loading ? "Processando..." : <>{mode==="forgot"?"Enviar instruções":mode==="reset"?"Atualizar senha":"Entrar no sistema"} <span>→</span></>}</button>
...
function AppShell({ user, onLogout }: { user:User; onLogout:()=>void }) {
  ...
  return <div className="app-shell">
    ...
    <div className="app-main"><header className="topbar"><button className="hamburger" onClick={()=>setMobile(true)}>☰</button><div className="greeting"><span>Olá, {user.name.split(" ")[0]}</span><b>{new Intl.DateTimeFormat("pt-BR",{weekday:"long",day:"2-digit",month:"long"}).format(new Date())}</b></div>
      <div className="top-actions">{user.role==="VISITANTE"&&<span className="demo-badge">◎ MODO VISITANTE · DADOS ILUSTRATIVOS</span>}<button className="bell">♢<i/></button><div className="user-chip"><span>{user.name.split(" ").map(x=>x[0]).join("").slice(0,2)}</span><p><b>{user.name}</b><small>{user.institutionalProfileName||user.role}</small></p></div><button className="logout" onClick={onLogout}>Sair ↗</button></div></header>
...
export default function RadarApp() {
  ...
  const loadAuthenticatedUser=async()=>{
    ...
    window.history.replaceState({},"","/dashboard");setReady(true);
  };
